#include "impl.h"

void DataBrowser::userLeave(const std::string &userId)
{
    m_dataReaders.erase(userId);
}

bool DataBrowser::getDataType1(const std::string &userId, std::vector<size_t> &returnValues) const
{
    auto dataRequest = [this, &returnValues](const std::unique_ptr<IDataSelector> &selector) {
      /*  auto dataType = [](const IDataSelector *selector, std::vector<size_t> &returnValues) {
            return selector->getDataType1(returnValues, 0);
        };*/
        auto dataType = std::bind( &IDataSelector::getDataType1, std::placeholders::_1, std::placeholders::_2, 0);

        return invokeDataRequest(dataType, selector, returnValues);
    };

    return safeCall(userId, dataRequest);
}

bool DataBrowser::getDataType2(std::vector<size_t> &returnValues, const std::string &userId) const
{
    auto dataRequest = [this, &returnValues](const std::unique_ptr<IDataSelector>& selector) {
        auto dataType = std::mem_fn(&IDataSelector::getDataType2);
        return invokeDataRequest(dataType, selector, returnValues);
    };

    return safeCall(userId, dataRequest);
}

bool DataBrowser::getDataType3(const std::string &userId, std::vector<std::string> &returnValues) const
{
    auto dataRequest = [this, &returnValues](const std::unique_ptr<IDataSelector>& selector) {
        std::deque<size_t> unprocessedResults {};
        auto dataType = std::mem_fn(&IDataSelector::getDataType3);
        //auto dataType = std::bind( &IDataSelector::getDataType3, std::placeholders::_1, std::placeholders::_2);
        bool success = invokeDataRequest(dataType, selector, unprocessedResults);
        returnValues = process(unprocessedResults);
        return success;
    };

    return safeCall(userId, dataRequest);
}

template<class T>
std::vector<std::string> DataBrowser::process(const T& source) const
{
    std::vector<std::string> results;
    std::transform(source.begin(), source.end(), std::back_inserter(results),
                   [](const size_t& value) { return std::to_string(value); });
    return results;
}
